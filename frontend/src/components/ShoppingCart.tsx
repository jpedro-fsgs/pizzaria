...
export default function ShoppingCart() {
    ...
    return products.map((product, i) => (
      <div key={i} className="flex justify-between items-center">
        <div>
          ...
          <p>{(product.preco * product.quantidade).toFixed(2)} $</p> {/* Formatação do preço */}
        </div>
  ...
  const total = products.reduce((acc, product) => acc + product.preco * product.quantidade, 0).toFixed(2); // Cálculo do total
  ...
  return (
    <div tabIndex={0} className="w-72 translate-x-16 card card-compact dropdown-content bg-base-100 z-[1] mt-3 shadow max-h-[80vh] overflow-y-auto">
      <div className="card-body">
        ...
        <span className="text-info font-bold">Total: ${total}</span> {/* Usando o total calculado */}
