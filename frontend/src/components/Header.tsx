...
const Header = () => {
  ...
  // Verifique se o contexto está definido
  if (!cartContext) {
    throw new Error("Header must be used within a CartProvider");
  }

  const { cart } = cartContext; // Desestruturação segura
          ...
          <nav className="hidden md:flex items-center space-x-6">
            ...
            <div className="relative">
              <div className="dropdown dropdown-end">
                <button tabIndex={0} className="btn btn-secondary w-52 flex items-center justify-between focus:outline-none" aria-label="Abrir carrinho de compras">
                  ...
                  <div className="indicator">
                    ...
                    <span className="badge badge-sm badge-info indicator-item">{cart.length > 0 ? `(${cart.reduce((acc, item) => acc + item.quantity, 0)})` : "0"}</span>
                  </div>
                </button>
